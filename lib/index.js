...
const OBSERVATION_ONLY_RULE = "Return only factual visual observations. Do not mention the visual model, provider, plugin, tool, endpoint, routing, or analysis process.";
...
function visionToolInstruction(media) {
...
function apply(ctx) {
	...
	ctx.on("agent/pre-step", async ({ agent, messages }, next) => {
		...
		if (batch === void 0 || batch.contextDeferred || !getConfig().autoVisionFallback || promptFromUserMessages(messages) === "") return decision;
		return {
			kind: "enter",
	...
	ctx.inject(["tools"], (tctx) => {
		tctx.tools.register(defineTool({
			name: "analyze_media",
			description: "读取用户当前消息附带的图片或影片，并把事实观察写入下一步上下文。调用一次后，直接回答用户关于媒体的问题；不要向用户提及或泄露视觉模型、Provider、插件、工具、端点、上下文注入、路由或内部分析流程。参数 prompt 为可选的分析要求。",
			parameters: { prompt: {
				type: "string",
			...
			async execute(args, exec) {
				...
				if (batch.contextDeferred) return {
					ok: true,
					contextInjected: true,
					mediaCount: batch.items.length
				};
				const explicitPrompt = args?.prompt?.trim() ?? "";
				const userPrompt = exec.agent === void 0 ? "" : promptFromUserMessages(exec.agent.session.deriveMessages());
				batch.analysis ??= analyzeMedia(getConfig(), explicitPrompt || visionPrompt(userPrompt), batch.items);
				const result = await batch.analysis;
				if (!result.ok || !result.text?.trim()) {
					batch.analysis = void 0;
					return result;
				}
