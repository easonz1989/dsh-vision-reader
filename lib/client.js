window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const submittedMedia = /* @__PURE__ */ new Map();
		const retainedPreviewUrls = /* @__PURE__ */ new Set();
		...
		function submittedMediaElement(record) {
		...
		function renderSubmittedMedia(sessionId) {
			const records = submittedMedia.get(sessionId);
			if (records === void 0 || records.length === 0) return;
			...
			for (const record of [...records].reverse()) {
				...
				const exactCandidate = record.nodeKey === "" ? null : document.querySelector(`[data-chat-anchor-key="${CSS.escape(record.nodeKey)}"][data-chat-flow-kind="user"]`);
				const userStack = (((exactCandidate !== null && (prompt === "" || normalizedText(exactCandidate.textContent ?? "").includes(prompt)) ? exactCandidate : null) ?? [...rows].reverse().find((candidate) => {
		...
		function scheduleSubmittedMediaRender(sessionId) {
		...
		function rememberSubmittedMedia(sessionId, prompt, nodeKey, items) {
			const records = submittedMedia.get(sessionId) ?? [];
			...
			records.push({
				...
				prompt,
				nodeKey,
				items: items.map(({ id, name, mime, previewUrl }) => ({
					id,
					name,
					mime,
					previewUrl
			...
			for (const record of removed) for (const item of record.items) {
		...
		function setClientMedia(sessionId, items) {
			...
			for (const item of previous) if (!keep.has(item.id) && !retainedPreviewUrls.has(item.previewUrl)) URL.revokeObjectURL(item.previewUrl);
...
`;
		...
		function MediaDock({ call, t, sessionId, session, useSession, useInput, inputActions }) {
			const media = useClientMedia(sessionId);
			const draft = useInput((state) => state.draft);
			const [removing, setRemoving] = (0, react.useState)("");
			const [error, setError] = (0, react.useState)("");
			const lastDraft = (0, react.useRef)(draft);
			const latestUserNodeKey = useSession((snapshot) => {
				for (let index = snapshot.chat.order.length - 1; index >= 0; index -= 1) {
					...
					if (key !== void 0 && snapshot.chat.nodes.get(key)?.kind === "user") return key;
				}
				return "";
			});
			(0, react.useEffect)(() => {
