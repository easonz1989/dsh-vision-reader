window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const EMPTY_CLIENT_MEDIA = [];
		const transcriptMediaCache = /* @__PURE__ */ new Map();
		const transcriptRecordsCache = /* @__PURE__ */ new Map();
		const transcriptRecordFlights = /* @__PURE__ */ new Map();
		const transcriptVersions = /* @__PURE__ */ new Map();
		const transcriptListeners = /* @__PURE__ */ new Map();
		...
		function invalidateTranscriptRecords(sessionId) {
		...
		function subscribeTranscriptVersion(sessionId, listener) {
			const listeners = transcriptListeners.get(sessionId) ?? /* @__PURE__ */ new Set();
			listeners.add(listener);
		...
		function useTranscriptVersion(sessionId) {
			return (0, react.useSyncExternalStore)((listener) => subscribeTranscriptVersion(sessionId, listener), () => transcriptVersions.get(sessionId) ?? 0, () => transcriptVersions.get(sessionId) ?? 0);
		}
		const dict = {
			zh: {
				nav: "视觉模型",
				...
				supported: "支持视觉",
				unsupported: "未声明视觉能力",
				autoLabel: "主模型不支持视觉时自动使用视觉插件",
				autoHint: "启用后，附加媒体会先由所选视觉模型读取，再把安全的文字分析交给主模型。",
				upload: "添加媒体",
				uploadBusy: "正在添加…",
			...
			en: {
				...
				autoLabel: "Auto use vision plugin when model doesn't have vision",
				autoHint: "Attached media is read by the selected vision model, then safe text analysis is passed to the primary model.",
				upload: "Add media",
				uploadBusy: "Adding…",
...
`;
		...
		function VLProviderSection({ call, t }) {
			...
			const [autoVisionFallback, setAutoVisionFallback] = (0, react.useState)(true);
			...
			const changeAutoVision = async () => {
				const next = !autoVisionFallback;
				setAutoVisionFallback(next);
				setError("");
				try {
					await call("set-auto-vision", { enabled: next });
				} catch (cause) {
					setAutoVisionFallback(!next);
					setError(messageOf(cause));
				}
				...
				children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
							...
							type: "button",
							role: "switch",
							"aria-checked": autoVisionFallback,
							className: `vr-switch ${autoVisionFallback ? "vr-switch-on" : ""}`,
							onClick: () => {
								changeAutoVision();
							}
						})]
		...
		*/
		function TranscriptMediaController({ call, sessionId, useSession }) {
			const nodeSignature = useSession((snapshot) => [...snapshot.chat.nodes.values()].map((node) => `${node.kind}\t${String(node.data.seq ?? "")}\t${node.key}`).join("\n"));
			const transcriptVersion = useTranscriptVersion(sessionId);
			const [records, setRecords] = (0, react.useState)([]);
			const [targets, setTargets] = (0, react.useState)(/* @__PURE__ */ new Map());
			(0, react.useEffect)(() => {
				let live = true;
				...
				if (!flight) {
					flight = (async () => {
						...
						if ((transcriptVersions.get(sessionId) ?? 0) === transcriptVersion) transcriptRecordsCache.set(sessionId, visible);
						return visible;
					})();
			...
			(0, react.useEffect)(() => {
				const frame = window.requestAnimationFrame(() => {
					...
					for (const record of records) {
						...
						if (!key) continue;
						const stack = flowItems.find((element) => element.getAttribute("data-chat-flow-key") === key)?.querySelector("[data-time-hover-root] > div");
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(react_jsx_runtime.Fragment, { children: records.map((record) => {
				const target = targets.get(record.userSeq);
				if (!target) return null;
				return (0, react_dom.createPortal)(/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					className: "vr-transcript-media",
					"data-count": record.items.length,
					"data-vision-reader-transcript": "",
					children: record.items.map((item) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("img", {
						className: "vr-transcript-image",
						src: item.dataUrl,
						alt: item.name
					}, item.index))
				}), target, `vision-reader-${record.userSeq}`);
			}) });
		}
		function MediaDock({ call, t, sessionId, useInput, inputActions }) {
