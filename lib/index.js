...
function apply(ctx) {
	...
	const persistTranscriptMedia = async (items) => {
		const stored = [];
	...
	const transcriptMediaFor = (sessionId) => {
		...
		cc.connection.rpc.handle("/vision-reader", async (endpoint, payload) => {
			try {
				...
				if (endpoint === "get-state") return {
					...
					value: {
						...
						transcriptMedia: transcriptMediaFor(sessionId).map((source) => ({
							...
							items: source.transcriptMedia.map((item, index) => ({
								index,
								name: item.name,
								mime: item.mime
							}))
				...
				if (endpoint === "read-transcript-media") {
					...
					const item = transcriptMediaFor(sessionId).find((source) => source.userSeq === userSeq)?.transcriptMedia[index];
